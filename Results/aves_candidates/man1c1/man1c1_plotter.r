####This script is customized to plot only RUNX2 plot in Aves
args = commandArgs(trailingOnly=TRUE)
if (length(args)<6) {
  stop("Please provide arguments like Rscript runx2_plotter.r inputfile treefile cladename genename windowsize codemloutput", call.=FALSE)
}
library(ape)
library(stringr)
library(TreeTools)
...
plot.phylo(t,use.edge.length=F,node.depth=2,label.offset=0.1,x.lim=phylolim,cex=2.2,edge.width=4.5)
...
rect(x1,y1,x2,y2,brder="transparent",col="burlywood3")}
}
} else {for(i in 1:nchar(s)){
aanow=data.frame(strsplit(s,split=""))[i,]
rect(x1,y1,x2,y2,border="transparent",col="burlywood3")
}
}
...
colnames(ld)=c("aa")
legend("bottomright",legend="Q",col="burlywood3",horiz=T,fill="burlywood3",inset=c(0.042,-0.01),bty="n",cex=2)
genenm=final$gene[1]
...
y1=(2*(ns1y1) - 1)-2
y2=(2*(ns1y2)+1)-2
rect(ns1x1,y1,ns1x2,y2,density=0,border=colvals[1],lwd=2.4)
...
y1=(2*(ns2y1) - 1)-2
y2=(2*(ns2y2)+1)-2
rect(ns2x1,y1,ns2x2,y2,density=0,border=colvals[2],lwd=2.4)
