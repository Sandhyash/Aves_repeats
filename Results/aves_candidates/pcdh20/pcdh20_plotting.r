...
par(mai=c(0.5,0,0.4,0),oma=c(0.5,0.4,0,0.2),xpd=T)
...
plot.phylo(t,use.edge.length=F,node.depth=2,label.offset=0.1,x.lim=phylolim,cex=3,edge.width=4.5)
...
rect(x1,y1,x2,y2,brder="transparent",col=meracol,lty="dashed",density=75,angle=45*i)}
...
aanow=data.frame(strsplit(s,split=""))[i,]
meracol=newcols[newcols$aa==aanow,2]
rect(x1,y1,x2,y2,border="transparent",col=meracol,lty="dashed",density=75,angle=45*i)
}
}
...
legend(max(final$alnlength)-200,-0.6,legend=aminocol$aa,col=aminocol$col,horiz=T,fill=aminocol$col,inset=c(0.042,-0.01),bty="n",cex=2.4)
genenm=final$gene[1]
m=final
