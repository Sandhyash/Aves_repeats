...
if (length(args)<6) {
  stop("Please provide arguments like Rscript repeatplotterwitheverything.r inputfile treefile cladename genename windowsize absrelresult codemlresult ", call.=FALSE)
}
library(ape)
library(stringr)
library(TreeTools)
...
abs=read.table(args[6])
colnames(abs)=c("clade","gene","species","pval")
ssfile=read.table(args[7])
...
if(cname=="All"){a=a1
absub=abs[abs$gene==gene,]
ss=ssfile[ssfile$gene==gene,] } else {
a<-a1[a1$clade==clade & a1$gene==gene,] 
absub=abs[abs$clade==clade & abs$gene==gene,] 
ss=ssfile[ssfile$clade==clade & ssfile$gene==gene,]}
final=a
t<-keep.tip(t1,unique(c(a$species,absub$species)))
pt=Preorder(t)
...
###Plotting aBSREL results in the rectangles
if (dim(absub)[1] > 0){
for(ps in 1:dim(absub)[1]){
g1=absub[ps,]
spnaam=g1[,"species"]
j=which(species$spnames==spnaam)
x1=0
x2=max(final$alnlength)
y1=((2*j)-0.5)-2
y2=((2*j)+0.5)-2
rect(x1,y1,x2,y2,col=rgb(0.67,0.13,1,alpha=0.3),border="transparent")}
}
...
x2=(3*l)+2
y2=ymax-1.5
rect(x1,y1,x2,y2,border="transparent",col=rgb(0.5,0,0,alpha=0.5)) } }
...
y1=(2*(ns1y1) - 1)-2
y2=(2*(ns1y2)+1)-2
rect(ns1x1,y1,ns1x2,y2,density=0,border=colvals[1])
...
y1=(2*(ns2y1) - 1)-2
y2=(2*(ns2y2)+1)-2
rect(ns2x1,y1,ns2x2,y2,density=0,border=colvals[2])
